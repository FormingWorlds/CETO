...
variables = ["T_surface", "T_eq", "moles_atm"]
arr_T_surface = np.arange(1, 6)*1000.0
arr_dT_eq = np.array([1000.0, 3000.0])
...
for i in range(len(arr_moles_atm)):
    ...
    for j in range(len(arr_dT_eq)):
        ...
        for k in range(len(arr_T_surface)):
            ...
            if count >= (int(len(arr_T_surface)*len(arr_dT_eq)*len(arr_moles_atm)))+1:
                break
            else:
                count += 1
                ...
                if count in skips:
                    ...
                    logging.info(f"Created {configname} with T_surface={T_surface_value}, T_eq={T_surface_value+dTeq_value}, Moles atm={moles_atm_value}")
