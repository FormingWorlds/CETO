...
for i in range(len(GR)):
    logging.debug("Gelman-Rubin statistic for {variable_keys[i]}: {GR[i]}")
...
for key in result:
    logging.info(f"result {key} = {result[key]}")
...
density_deficit_B = (8.7*wtpcs["wt_H_metal"]+1.2*wtpcs["wt_O_metal"]+0.8*wtpcs["wt_Si_metal"]) # in %; taken from Young's code, evidently 'deficit based on experimental values'
...
outfile.write("%10.5e " %density_deficit_A); outfile.write(" # Density deficit by comparison with pure Fe in %\n")
outfile.write("%10.5e " %density_deficit_B); outfile.write(" # Density deficit by experimental values in %\n")
